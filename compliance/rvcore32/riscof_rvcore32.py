...
class rvcore32(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # self.dut_exe = os.path.join(config['PATH'] if 'PATH' in config else "","rvcore32")
        self.dut_exe = "/home/mj/intelFPGA_lite/21.1/questa_fse/bin/vsim"
    ...
    def initialise(self, suite, work_dir, archtest_env):
       ...
       self.objcpy = 'riscv{1}-unknown-elf-objcopy {0} -O ihex /home/mj/Documents/uob/itce497/rvcore32/programs/prog.hex'
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      # for rvcore32 start building the '--isa' argument. the self.isa is dutnmae specific and may not be
      # useful for all DUTs
    ...
    def runTests(self, testList):
      ...
      for testname in testList:
          ...
          objcpy = self.objcpy.format(compile_macros,64)
	  # if the user wants to disable running the tests and only compile the tests, then
	  # the "else" clause is executed below assigning the sim command to simple no action
	  # echo statement.
          if self.target_run:
            # set up the simulation command. Template is for spike. Please change.
            simcmd = self.dut_exe + ' -c -do \"do /home/mj/Documents/uob/itce497/rvcore32/programs/cc.tcl\"' #--isa={0} +signature={1} +signature-granularity=4 {2}'.format(self.isa, sig_file, elf)
          else:
            simcmd = 'echo "NO RUN"'

          # concatenate all commands that need to be executed within a make-target.
          sigbeginning = "sed -i '1i RVMODEL_DATA_BEGIN' {0}".format(sig_file)
          sigending = "echo \"RVMODEL_DATA_END\n\" >> {0}".format(sig_file)
          execute = '@cd {0}; {1}; {2}; {3}; {4}; {5};'.format(testentry['work_dir'], cmd, objcpy, simcmd, sigbeginning, sigending)
